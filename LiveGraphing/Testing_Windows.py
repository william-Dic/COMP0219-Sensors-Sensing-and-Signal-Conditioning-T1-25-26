                    ...
                    if not line:
                        ...
                        if (now - self._last_data_ts) > 2.0:
                            try:
                                ...
                                present = self._port_name in ports
                                if not present:
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, devices, max_points=3000, refresh_ms=30, window_seconds=15, max_speed=10.0, min_speed=0.0):
        super().__init__()
        self.setWindowTitle("NUCLEO Live Plot (Windows)")
        self.resize(900, 500)
        ...
        #     print("Waiting for additional COM* device to attach...")
    ...
    def refresh(self):
        ...
        if not getattr(self, '_log_mode_applied', False) and getattr(self, 'y_log', False):
            if self.min_speed <= 0:
                self.min_speed = 1e-6
            try:
        ...
        try:
            ...
            for port in self.curves.keys():
                ...
                if t_cut is None:
                    ...
                    # Consider only values within the visible time window
                    local_max = None
    ...
    def attach_device(self, port: str, name: str = 'student', baud: int = 115200):
        ...
        if _modem_group(port) != self.ground_truth_group:
            # If a previous student existed, remove it before assigning new
    ...
    def scan_and_attach_new_devices(self):
        try:
            ...
            if available:
                # Attach the first new device we see
    ...
    def on_disconnected(self, port: str):
        try:
            ...
            # Reset student assignment if this was the student
    ...
    def remove_device(self, port: str):
        ...
        # Mark port as no longer known so scanner can find a replacement
...
def _modem_group(path: str):
    # On Windows, grouping is not needed; just return the path
    return path

def _list_available_device_paths():
    # Windows: list all COM* ports reported by pyserial
    try:
...
def _resolve_devices_from_config(config_devices):
    ...
    for d in (config_devices or []):
        ...
        port = d.get('port')
        if not port:
            print(f"ERROR: Device '{name}' is missing a 'port' in config.yaml")
            continue
        resolved.append({'name': name, 'port': port, 'baud': baud})

    if len(resolved) != 2:
        print("ERROR: Exactly two devices must be configured in config.yaml under 'devices'.")
        print("Example:")
        print("devices:")
        print("  - name: Ground Truth")
        print("    port: COM3")
        print("    baud: 115200")
        print("  - name: Student")
        print("    port: COM4")
